...
def plot_diadic_pattern(dataset, **plot_kwargs):
    ...
    for i in range(layout[0]):
        for j in range(layout[1]):
            ...
            ax[selector].bar(label, list(data.values()), **plot_kwargs)
            ax[selector].set_title(title)
...
def plot_bar_diadic_pattern(dataset, **plot_kwargs):
    ...
    for i in range(len(possible_phenotype)-1):
        ...
        ax.bar(label, np.array(list(data.values()))/2, bottom=bottom, label=ph, **plot_kwargs)
...
def plot_phenotype_combination_per_link(link_type, combination_dt, th=0):
    ...
    ax.set_title("Phenotype combinations distribution for {0} with threshold {1}".format(diadic_link_to_text[link_type], th))
...
def plot_triadic_pattern_phenotype_from_hist(freq_dict_hist, triangle_list, randomized_dict_hist=None, err=None, triangle_only=False, **plot_kwargs):
    ...
    if not randomized_dict_hist is None:
        rand_values = randomized_dict_hist["Number"]
        if triangle_only:
            rand_values = rand_values[3::]
        x = np.arange(0, values.size*2, 2)
        ax.bar(x, rand_values, width=1, align="center", facecolor=(0,0,0,0), edgecolor="k", label="Randomized")
